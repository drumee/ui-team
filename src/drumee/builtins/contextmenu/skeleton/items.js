const __button = function (ui, trigger, k) {
  ...
  const item = (service, content, extra = {}) =>
    Skeletons.Box.X({
      service,
      ...extra,
      kids: [
        Skeletons.Button.Svg({
          ico: icons[k],
          className: "contextmenu-item__icon",
        }),
  ...
  let a = {
    ...
    seeChatThreads: item(_a.chat, LOCALE.SEE_CHAT_THREADS),
    ...
    duplicate: item(_a.duplicate, LOCALE.MAKE_A_COPY),
    ...
    export: item("export-to-server", LOCALE.EXPORT_TO_SERVER, {
      type: _a.export,
    }),
    exportHidden: item(_a.none, LOCALE.EXPORT_TO_SERVER, {
      type: _a.export,
      dataset: { state: _a.disable },
    }),
    fullscreen: item("toggle-fullscreen", LOCALE.FULLSCREEN),
    helpdesk: item(_a.helpdesk, LOCALE.HELPDESK),
    import: item("import-from-server", LOCALE.IMPORT_FROM_SERVER, {
      type: _a.import,
    }),
    importHidden: item(_a.none, LOCALE.IMPORT_FROM_SERVER, {
      type: _a.import,
      dataset: { state: _a.disable },
    }),
    info: item(_e.settings, LOCALE.GET_INFO, { type: _a.info }),
    link: item(_a.link, LOCALE.SHARE_LINK),
    linkToTaskTracker: item(
      "link-to-task-tracker",
      LOCALE.LINK_TO_TASK_TRACKER,
    ),
