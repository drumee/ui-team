...
// panel, one after the other. Invite outcomes report inline now (a67469ef),
// so what is left on a modal is a member mutation the server refused — same
// bare card, same 600px.
//
...
test("the panel actually has messages to route", () => {
  ...
  // Five today: two on the role change, three on the removal. The floor is
  // what stops test 1 passing because every call was deleted.
  assert.ok(routed.length >= 5,
    `expected the panel's modal messages to go through _notice, found ${routed.length}`);
});
