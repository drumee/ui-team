...
// ===========================================================
  onBeforeAddChild(child, pn, section) {
    ...
    if (child.get('sys_pn') === _a.image) {
      let url;
      if (this.get(_a.type) === _a.hub) {
        url = `url('?m=hub.photo&id=${Host.get(_a.id)}')`;
      } else {
        url = `url('?m=drumate.photo&id=${Visitor.get(_a.id)}')`;
      }
      const bg = require('options/css/background')(this, url);
