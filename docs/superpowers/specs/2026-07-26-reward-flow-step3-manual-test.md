...
| 5 | Reach step 3 after any of the above | Card centred, no cutout on the topbar Upload button, button reads **Open workspace** |
...
| 14 | Reload on step 3 after `localStorage.removeItem("reward_workspace")` | Legacy variant: cutout on the desk topbar Upload button, button reads **Upload** |
...
| 16 | Open a workspace where the user is view-only, then reach step 3 | The **+ New** pill never appears (permission-gated `data-visible`); after ~4 s the flow drops to the legacy variant |
| 17 | Step 1 walkthrough end to end (add → menu → form → perm), plus Back at each | Unchanged from before this branch — the guide-core extraction regression check |
